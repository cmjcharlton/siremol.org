...
```{r}
text_lines = c("To be, or not to be--that is the question:",
...
```{r}
soliloquy <- tibble(line = 1:5, text = text_lines)
soliloquy
```
...
```{r}
tokens <- soliloquy |> unnest_tokens(word, text)
tokens
```
...
have the line number (from 1 to the number of lines), while the lines
...
to be represented as [factors](https://r4ds.had.co.nz/factors.html).
We do this using the [fct_reorder](https://forcats.tidyverse.org/reference/fct_reorder.html)
function from tidyverse's [forcats](https://forcats.tidyverse.org/index.html) package.
This will order the factors according to the number of occurrences
...
```{r}
counts <- counts |> mutate(word = fct_reorder(word, n))
```
...
the number of occurrences on the x axis, the word on the y axis,
