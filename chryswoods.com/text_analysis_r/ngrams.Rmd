...
or `word2` have a word in `stop_words`. While we could use `anti_join` again:
...
This involves joining twice. We can instead apply a single
filter, excluding rows where `word1` is in the `word` column of `stop_words`
or where `word2` is in the `word` column of `stop_words`. Note that
`%in%` is an operator that returns a vector of logical values indicating
whether each elements in the first vector exists in the second.
...
using the function `drop_na`, which when called without arguments removes
any rows that contain an `NA`.
