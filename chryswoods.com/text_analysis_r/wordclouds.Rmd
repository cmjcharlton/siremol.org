...
to plot the words in order of decreasing frequency, rather than
a random order).
...
You could alternatively use `%$%`, the "exposition" pipe, from
magrittr:

```{r}
library(magrittr)
sherlock_counts %$%
    wordcloud(word, n, min.freq = 25, random.order = FALSE)
```

Or the `with` function from base R to make this part of a pipeline, e.g.

```{r}
sherlock_counts |>
    with(wordcloud(word, n, min.freq = 25, random.order = FALSE))
```

Note that this has to be at the end of the pipeline as the data is not
passed further on after these.
