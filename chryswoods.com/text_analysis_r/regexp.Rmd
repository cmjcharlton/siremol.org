...
```{r}
hamlet %>% filter(str_detect(text, "To"))
```
...
```{r}
hamlet %>% filter(str_detect(text, "To", negate = TRUE))
```
...
```{r}
hamlet %>% filter(str_detect(text, regex("To", ignore_case = TRUE)))
```
...
```{r}
hamlet %>% filter(str_detect(text, "\\bthe\\b"))
```
...
```{r}
hamlet %>% filter(str_detect(text, "the\\w"))
```
...
```{r}
hamlet %>% filter(str_detect(text, "\\bthe\\w"))
```
...
```{r}
hamlet %>% filter(str_detect(text, "th[aiy]"))
```
...
```{r}
hamlet %>% filter(str_detect(text, "\\w{10,12}"))
```
...
```{r}
hamlet %>% filter(str_detect(text, "(monologue)|(dream)"))
```
...
```{r}
hamlet %>% filter(str_detect(text, "\\b(T|t)he(re)?\\b"))
```
...
```{r}
hamlet %>% filter(str_detect(text, "^The"))
```
...
```{r}
hamlet %>% filter(str_detect(text, "on$"))
```
...
```{r}
text_line <- hamlet[4, 2]

str_replace(text_line, "be", "banana")
str_replace_all(text_line, "be", "banana")
```

You can also choose to match the string to replace using a regex, e.g.

```{r}
str_replace_all(text_line, regex("to be", ignore_case = TRUE), "ice-cream")
```
...
```{r}
hamlet %>% mutate(text = str_replace_all(text, "be", "banana"))
```

```{r}
hamlet %>% mutate(text = str_replace_all(text, regex("\\bthe\\b", ignore_case = TRUE), "banana"))
```
...
else who looks at or relies on your code. If you don't believe me then
have a read through [this](https://stackoverflow.com/questions/201323/how-can-i-validate-an-email-address-using-a-regular-expression)
stack overflow discussion on validating email address using regular expressions.
