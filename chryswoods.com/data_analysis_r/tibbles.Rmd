...
A `tibble` represents a two dimensional table of data.
...
This has created a `tibble` that we have assigned to the object `census`.
The column names are the keys (`City`, `year` and `pop`), while the 
data for each column is given in the values (the vectors).
...
Note that if you do not specify otherwise R will default to interpreting numbers
as floating point (`dbl`). While this is correct for the `pop` (population) column, 
this is the wrong choice for the year. A better choice would be an integer. To 
correct this, use the L suffix on the value for the year column;
...
will return the first column, so is identical to `census %>% select(City)`.

You can also extract multiple columns by specifying more than one, e.g.

```R
census %>% select(City, year)
```

will return a `tibble` with the `City` and `year` columns.

To access data by rows, you need to use the slice function, e.g.
...
```R
census %>% slice(seq(2, 10, 2))
```

would return the even rows from 2 to 10.

You can combine these to specific rows and columns, e.g.
...
can extract the data for a single column as a vector via the `pull` function, e.g.
...
and can then extract data from those vectors by combining these functions, e.g.
...
New columns can be added to a `tibble` simply by using the mutate function;

```R
census <- census %>%
    mutate(continental = (City != "London"))
census
```
