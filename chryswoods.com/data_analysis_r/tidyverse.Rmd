...
If you have ever used a relational database you may notice the similarities
with this. In fact the tidyverse includes a package named `dbplyr` which
allows you to operate directly on database tables in a very similar way
to how it does with tibbles.

It should be noted that the opposite of "tidy" data is not "messy" data. There
are often a number of reasons to store data in non-tidy structures, but doing so
makes a good default starting point. You can read a discussion of this at the
page [Non-tidy data](https://simplystatistics.org/2016/02/17/non-tidy-data/).
...
You may wonder why character variables are not automatically converted for you
when initially loading the data file. This was the default in base R prior to
version 4.0.0, and you can do so by specifying `col_factor()` as the `col_types`
when using `readr`. This is another example of the tidyverse "Lazy" philosophy
where such operations must be explicitly specified by the user. This makes sense
for a number of reasons, for example you might need to tidy the values before they
they can be converted or you may want to analyse the variable as text, as
demonstrated in the [Text and Sentiment Analysis in R](https://chryswoods.com/text_analysis_r/index.html)
workshop.

## Further reading
* [Advanced R - Factor](https://adv-r.hadley.nz/vectors-chap.html#factors)
* [R Language Definition - Factor](https://cran.r-project.org/doc/manuals/r-release/R-lang.html#Factors)
