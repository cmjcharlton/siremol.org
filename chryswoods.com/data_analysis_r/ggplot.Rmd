...
```R
historical_temperature |> ggplot(aes(x = year, y = temperature)) + geom_point()
```
...
```R
data |> ggplot( aesthetic ) + layer1 + layer2 + layer3...
```

The `data` is a `tibble` (although providing a `data.frame` here works
equally well) containing tidy data with one observation per row,
...
historical_temperature |>
    group_by(decade) |>
    summarise(temperature = mean(temperature)) |>
    ggplot(aes(x = decade, y = temperature)) + 
    geom_line()
```
...
```R
historical_temperature %>%
    ggplot(aes(x = decade, y = temperature, group = decade)) + 
    geom_violin()
```
...
This will save the last plot drawn to a file, with file name, size,
