...
maybe it's from a colleague. In an ideal world the file will be perfectly 
...
This means that we can take our URL and pass it directly (or via an object) to the function:
...
(this is mostly because I've constructed the `city_pop.csv` file to be as 
obtuse as possible). It's making a lot of assumptions about the structure 
of the file but in general it's taking quite a naïve approach (this is in keeping
with the tidyverse philosophy of functions being "Lazy", forcing the user to be
explicit with what they want it to do).
...
This is because I have used `read_csv`, which assumes variable values are
separated by commas. This is a special case of the more general `read_delim`
...
data of the appropriate type. `read_delim` will guess which parser to use, with
...
This is sensible because the double data type allows storing a larger range of
integer values and so is much less likely to truncate the input. We know however
in this case that the year will take a fairly small range of values and we can
therefore save some memory by using the integer type.
...
Finally, we want to assign this `tibble` to an object, called `census`;
