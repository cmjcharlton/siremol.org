...
contains the mean average temperature. Notice the back ticks in the output
column name. This is because we have chosen a variable name that is not
syntactically valid (it contains a space). Tibbles allow this, but you may
encounter problems if you then use this output for further analysis. If you want
to ensure that a name is valid then R provides a [make.names](https://rdrr.io/r/base/make.names.html)
...
historical_temperature |>
    ...
    group_by(month) |>
    summarise("18th Century" = mean(temperature))
```
...
historical_temperature |>
    filter(year >= 2000) |>
    group_by(month) |>
    summarise("21st Century" = mean(temperature))
```
