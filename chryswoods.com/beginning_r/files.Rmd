...
thing is to describe where to find the file. This is done by providing a string
with the file name as the first argument for the `readLines` function. Using 
this the function will create a connection to the file (called `file.R`), read
the contents into a character vector named `file_lines` and then close the file.

If this file does not exist, then `readLines` will print an error
and exit the script, e.g.

```R
file_lines <- readLines("does_not_exist.R")
```
