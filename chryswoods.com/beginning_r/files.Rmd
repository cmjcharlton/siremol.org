...
```
[1] "file_handle <- file(\"file.R\")"
[1] "file_lines <- readLines(file_handle)"
[1] "close(file_handle)"
[1] ""
[1] "for (file_line in file_lines) {"
[1] "  print(file_line)"
[1] "}"
```
...
of the lines. These are returned as a character vector, which we assign to a 
variable called `file_lines`, via `file_lines <- readLines(file("file.R"))`.

If this file does not exist, then `readLines` will print an error
and exit the script, e.g.

```R
file_handle <- file("does_not_exist.R")
file_lines <- readLines(file_handle)
close(file_handle)
```
...
Error in readLines(file_handle) : cannot open the connection
In addition: Warning message:
In readLines(file_handle) :
  cannot open file 'does_not_exist.R': No such file or directory
Execution halted
```

If the file does exist, then it is opened and all of its lines read into 
memory. If you forget to close the file then R will close it automatically
(with a warning) some time after there are no longer any references to the
connection or when the script ends.

In the next line (`for (file_line in file_lines)`) we are looping over the lines 
of the file. This loop looks just like those we used a few chapters previously, 
because `file_lines` is just a character vector containing the lines of the file. 
We assign the string containing the line from the file to the variable `file_line`.

Finally, we print the string `file_line`. Note that R will automatically
...
You may be wondering where the `[1]` comes from. When R displays a vector it starts
each line with the index that corresponds to the first element displayed on the line.
As in this case print is displaying a character vector with one element this will 
always be `[1]'. You could experiment with longer vectors to become more familiar 
with this.
...
```
Error in file_line + 17L : non-numeric argument to binary operator
Execution halted
```

This is telling us that there is an error occurring when trying to add 17 to the 
...
value. The only two things involved in this operation are `file_line` and `17L`. 
We know that `17L` is an integer, and so is a numeric, so `file_line` must be 
...
>
> hint: Make an scalar before the loop, initially set to zero and 
...
>
> Collect the statistics into a summary vector with names `sum`, `count` and `mean`.
>
