...
surprised that all the objects created within a loop are still available after
it has ended, for example if you ran the code:

```R
print(word)
```

After running the loop above you would see the output:

```
[1] "R"
```

which is the last value this item contained. This happens because loops in R do
not run in their own environment unless you wrap them with the `local` function.
...
## Further reading
* [Advanced R - Loops](https://adv-r.hadley.nz/control-flow.html#loops)
* [R Language Definition - Looping](https://cran.r-project.org/doc/manuals/r-release/R-lang.html#Looping)
