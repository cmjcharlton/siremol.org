...
is a sign to other people (and text editors) that this text file contains commands 
...
exactly what the function should do. R is a very open environment and you can usually
see how a function works by typing in its name without the parentheses.
...
Note that the default behaviour of R is to automatically call the `print` function
on the final result of each line. This means that you could have omitted the print
function and replaced the script content with:

```R
"Hello from R"
```

to get the same result. This use is very common with R users and so you are likely
to encounter it when reading code from others, however we shall be using
`print` within this course to make the intent explicit.
